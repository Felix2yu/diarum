...
- **周期报告改为按周期键存储** — `period_analyses` 表新增 `period_key` 字段，周/月/年报告以"第一周、第一月……"的特殊日期字段寻址（周 `2026-W36`（ISO 8601）/ 月 `2026-09` / 年 `2026`），不再以时间区间为主键；`start_date` / `end_date` 仅为推导出的展示与取数区间。升级时检测到旧表结构会**直接重建该表，旧的周期总结数据不会被迁移**，请提前导出备份（导出/导入已携带 `period_key`）。

### ✨ 新增 Features

- **年分析（年报）** — 新增按年度的 AI 周期分析，日历主按钮行与年视图工具栏均可入口；日历选择器新增「年报」页签，可直接选择某年的年报
- **周报/月报手动填写** — 周/月/年分析弹窗支持「填写报告」，无需 AI 直接手写周记分析/月记分析/年记分析并保存，与 AI 生成结果同样可在历史分析中查看、随时改写
- **日历选择器直达报告** — 年月选择器扩展为「月份 / 周报 / 月报 / 年报」页签：「周报」按 ISO 8601 周编号（第 1 周~第 53 周）列出全年各周并标注本周，点选即打开对应周报；「月报」点选月份直接打开月报

### 🔧 技术改进

- **ISO 8601 周编号** — 前端新增周号计算（含第一个星期四的周为第 1 周、52/53 周年判断、跨年归属），后端 `POST/PUT /api/v1/ai/analysis` 支持以周期键生成/保存报告并服务端推导日期范围，非法周期键（如 `2027-W53`）返回 400
- **自定义分析入口弱化** — 自定义（任意日期范围 + 关键词）分析保留，但主按钮从日历移除，收敛至「历史分析」弹窗内的「+ 自定义分析」入口，仅供旅行等非整周/整月时间段使用
- **导出/导入** — 分析报告的 JSON 与 Markdown 导出/导入携带 `period_key`；`analysis/` 目录下周/月/年报告的文件名改用周期键（如 `2026-W36.md`、`2026-09.md`、`2026.md`），自定义分析仍按日期区间命名
...
> 发布日期：2026-06-22
> 对比基线：`v1.7 (aac1927)` → `v1.8 (1701e19)`

### ✨ 新增 Features

- **心情滑动条** — 心情评分从 emoji 选择改为 1-5 数值滑动条，类似苹果健康样式，更直观
- **心情状态多选** — 每个心情等级对应一组心情状态标签（如开心→兴奋、感恩、平静），支持多选
- **情景标签** — 新增情景标签系统，记录日记发生的情景（如工作、运动、社交等）
- **心情筛选页** — 新增独立的 `/filter` 页面，支持按心情等级、心情状态、情景标签组合筛选日记
- **导航栏心情入口** — 顶部导航栏新增爱心图标，快速跳转到心情筛选页
- **导出分析报告** — 导出时新增「分析报告」选项，支持导出已保存的 AI 分析报告（PeriodAnalysis）
- **导入拖放支持** — 导入区域支持拖放 `.zip` 文件，松手即触发导入

### 🎨 UI 优化

- **导出 md 标签中文化** — 导出的 markdown 文件中 `**Mood:**` → `**心情：**`，`**Weather:**` → `**天气：**`，新增 **心情状态** 和 **情景** 标签
- **导出文件名简化** — 导出的 md 文件名移除心情数字后缀（如 `2026-01-15_3.md` → `2026-01-15.md`）
- **导出选项持久化** — 导出配置（日期范围、勾选状态）保存到 localStorage，刷新后保留
- **同步状态中文化** — 右下角同步提示从 `Saved` / `Saving...` 改为 `已保存` / `正在保存...`
- **深色模式开关修复** — 设置页开关关闭状态在深色模式下更清晰（`bg-muted` → `bg-border`）

### 🐛 Bug 修复

- **心情状态/情景刷新丢失** — 修复同步时遗漏 `mood_states` 和 `scenarios` 字段导致刷新后丢失的问题
- **搜索摘要中文乱码** — 搜索结果摘要按 rune 截断，修复中文字符被截断的问题
- **导入 md 兼容性** — 导入时兼容中英文标签（`**心情：**` / `**Mood:**`），向后兼容旧版导出文件

### 🔧 技术改进

- **GitHub Actions 升级** — CI 工作流升级到 v6，解决 Node.js 20 弃用警告
- **心情数据模型** — `diaries` 表新增 `mood_states`（JSON 数组）和 `scenarios`（JSON 数组）字段，自动迁移
...
- **导入 .md 文件** — 导入功能支持 ZIP 中包含多个 `.md` 文件（无需 `diarum_export.json`），自动从文件名或 `# YYYY-MM-DD` 标题提取日期，解析 `**Mood:**` 和 `**Weather:**` 元数据
- **导入冲突对比** — 日期冲突时不再静默跳过，而是展示差异对比界面：支持逐行差异高亮（绿色新增/红色删除）和并排查看两种视图，用户可手动选择保留现有版本或替换为导入版本
- **导入详细结果** — 导入完成后显示每条日记的详细状态（已导入/日期冲突/失败及原因）
