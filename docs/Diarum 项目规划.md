...
| 模块 | 功能点 | 详细描述 | 状态 |
| :--- | :--- | :--- | :--- |
| **高级导航** | **往昔今朝** | 在日历视图中点击"往昔今朝"按钮，弹窗展示同年-月-日的所有历史日记（日期、心情、天气、内容预览、标签）。API：`/api/v1/diaries/on-this-day`。 | ✅ 已实现 |
| | **时空穿越** | 在日历视图中点击"时空穿越"按钮，随机抽取一篇历史日记，支持"再抽一次"和"查看完整日记"。API：`/api/v1/diaries/random`。 | ✅ 已实现 |
...
| 高级导航（V2） | 90% | 标签/心情/天气/往昔今朝/时空穿越 均已实现；仅剩更深入的导航增强（如按年回顾）可按需扩展。 |
...
| `GET` | `/api/v1/diaries/on-this-day` | 查询并返回"同月-日、不同年"的历史日记（默认以今日为基准，可带 `?date=YYYY-MM-DD` 覆盖）。 |
| `GET` | `/api/v1/diaries/random` | 从 content/mood/weather/tags 至少一项非空的日记中随机抽取一条（可带 `?exclude=YYYY-MM-DD` 排除指定日期）。 |
