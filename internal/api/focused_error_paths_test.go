...
func TestAIRoutesHappyPath(t *testing.T) {
	...
	if _, err := s.InsertImportedDiary(user.ID, "", "2024-01-15", "some content", 4, "", nil); err != nil {
...
func TestFocusedExportImportEdges(t *testing.T) {
	...
	if _, err := s.InsertImportedDiary(user.ID, "export-old", "2024-01-01", "old diary", 0, "", nil); err != nil {
		t.Fatalf("InsertImportedDiary old: %v", err)
	}
	if _, err := s.InsertImportedDiary(user.ID, "export-new", "2024-02-02", "new diary", 0, "", nil); err != nil {
...
func TestResolveConflictRoutes(t *testing.T) {
	...
	if _, err := s.InsertImportedDiary(user.ID, "old", "2024-06-01", "old content", 2, "", nil); err != nil {
		t.Fatalf("InsertImportedDiary: %v", err)
	}
