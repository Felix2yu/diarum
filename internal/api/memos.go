...
func upsertMemosBlock(s *store.Store, userID, memoID, date, block string) (bool, error) {
	...
	_, _, err = s.UpsertDiary(userID, store.DateOnly(diary.Date), content, diary.Mood, nil, diary.Weather, diary.Tags)
	return err == nil, err
}
...
func removeMemosBlock(s *store.Store, userID, memoID, date string) (bool, error) {
	...
	_, _, err = s.UpsertDiary(userID, store.DateOnly(diary.Date), strings.TrimSpace(content), diary.Mood, nil, diary.Weather, diary.Tags)
	return err == nil, err
}
