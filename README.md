...
- 📤 **一键分享** - 轻点即可分享日记内容
- 📥 **灵活导入** - 支持导入 `.json` 文件、导出 ZIP 或包含 `.md` 文件的 ZIP，日期冲突时可差异对比并手动选择保留版本
- 🔄 **离线与自动同步** - 完整离线支持，自动缓存同步，实时查看数据同步状态
- 🏷️ **多标签系统** - 为日记添加标签，标签云可视化，按标签筛选与搜索
- 🗣️ **语音输入** - 支持录音转文字，快速记录
- 🤖 **AI 文本整理** - 一键去语气词、纠错、自动分段，支持自定义 prompt
- 📊 **AI 周期分析** - 周报/月报/年报按周期键存储与管理（周报采用 ISO 8601 周编号，如 2026-W36），支持 AI 生成或手动填写；历史报告管理，另保留自定义时间范围与关键词分析
